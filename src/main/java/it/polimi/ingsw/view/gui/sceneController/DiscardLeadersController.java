...
import it.polimi.ingsw.network.message.request.DiscardLeadersRequest;
...
/**
 * Handles scene to discard Leader cards
 */
public class DiscardLeadersController extends GuiController {
    private final List<Integer> selectedLeaders = new ArrayList<>();
    private final List<String> inputLeadersPath = new ArrayList<>();

    @FXML
    public BorderPane discardLeadersPane;
    ...
    public void initialize(){
        GuiController.draggableStage(discardLeadersPane,actionStage,null);
    ...
    public void setClickedEvent(int index){
        switch (index){
            case 0:
    ...
    public void onClick(MouseEvent event) {
        ...
        discardBtn.setDisable(!(selectedLeaders.size() > 0));
    }
    ...
    public void discardUpdate(Event event) {
        ...
        closeActionStage();
        sender.send(new DiscardLeadersRequest(nickname, selectedLeaders));
    }

    @FXML
    public void skipUpdate(Event event) {
        discardLeadersPane.setDisable(true);
        skipBtn.setDisable(true);
        List<Integer> nullLeaders = new ArrayList<>(){{add(0);}};
        closeActionStage();
        sender.send(new DiscardLeadersRequest(nickname, nullLeaders));
